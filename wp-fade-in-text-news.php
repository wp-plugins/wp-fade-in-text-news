...
Tags: Wordpress, plugin, widget, fadein, fade-in, fade in, announcement, text, message, news,
*/
...
function FadeIn() 
{
	...
	?>
  <!--  <script type="text/javascript" src="<?php //echo get_option('siteurl'); ?>/wp-content/plugins/wp-fade-in-text-news/wp-fade-in-text-news.js"></script>-->
...
function FadeIn_add_javascript_files() 
{
	if (!is_admin())
	{
		wp_enqueue_script( 'javascript', get_option('siteurl').'/wp-content/plugins/wp-fade-in-text-news/wp-fade-in-text-news.js');
	}	
}
...
function FadeIn_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_FadeIn_TABLE . "'") != WP_FadeIn_TABLE) 
	{
		...
		$sSql = $iIns . "VALUES ('Everybody loves fading in and out; this wp fade in text news plugin will create the fade-in and out effect in the text.','http://www.gopiplus.com/work/', '1', 'YES', 'sample', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		$sSql = $iIns . "VALUES ('It is an superb excellent way to transition between announcements. Admin can add more announcement using plugin text management.','http://www.gopiplus.com/work/', '2', 'YES', 'sample', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		$sSql = $iIns . "VALUES ('Only website admin and the user have the administrator privilege can see and change the setting in the website administration area.','http://www.gopiplus.com/work/', '3', 'YES', 'sample', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		$sSql = $iIns . "VALUES ('Everybody loves fading in and out; this wp fade in text news plugin will create the fade-in and out effect in the text.','http://www.gopiplus.com/work/', '4', 'YES', 'widget', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		$sSql = $iIns . "VALUES ('Only website admin and the user have the administrator privilege can see and change the setting in the website administration area.','http://www.gopiplus.com/work/', '5', 'YES', 'widget', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
	}
...
{
	echo '<p>Fade in text news.<br> To change the setting/To manage content goto Fade in text news link under SETTINGS tab.';
...
<a target="_blank" href='http://www.gopiplus.com/work/2011/04/22/wordpress-plugin-wp-fadein-text-news/'>Click here</a> to see More information.<br />
<?php
}
...
function FadeIn_admin_options() 
{
	...
	?>
<div class="wrap">
  <h2><?php echo wp_specialchars("Fade in text news"); ?></h2>
</div>
<?php
...
add_filter('the_content','FadeIn_Show_Filter');

function FadeIn_Show_Filter($content)
{
	return 	preg_replace_callback('/\[FADEIN_TEXT_NEWS (.*?)\]/sim','FadeIn_Show_Filter_Callback',$content);
}

function FadeIn_Show_Filter_Callback($matches) 
{
	global $wpdb;
	...
	//$JaFade = $JaFade . "<script type='text/javascript' src='".get_option('siteurl')."/wp-content/plugins/wp-fade-in-text-news/wp-fade-in-text-news.js'><script>";
...
function FadeIn_init()
{
	...
	{
		register_sidebar_widget('Fade in text news', 'FadeIn_widget');
	}
	
	if(function_exists('register_widget_control')) 
	{
		register_widget_control(array('Fade in text news', 'widgets'), 'FadeIn_control');
	} 
}
